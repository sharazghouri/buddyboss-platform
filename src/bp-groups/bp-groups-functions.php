...
function bp_groups_memberships_personal_data_exporter( $email_address, $page ) {
	...
	foreach ( $memberships as $membership ) {
		...
		if ( $membership->inviter_id ) {
			$item_data[] = array(
				'name'  => __( 'Invited By', 'buddyboss' ),
				'value' => bp_core_get_userlink( $membership->inviter_id ),
			);
		}

		if ( $group->creator_id === $user->ID ) {
			$group_role = __( 'Creator', 'buddyboss' );
		} elseif ( $membership->is_admin ) {
			$group_role = __( 'Organizer', 'buddyboss' );
		} elseif ( $membership->is_mod ) {
			$group_role = __( 'Moderator', 'buddyboss' );
		} else {
			$group_role = __( 'Member', 'buddyboss' );
		}

		$item_data[] = array(
			'name'  => __( 'Group Role', 'buddyboss' ),
			'value' => $group_role,
		);

		$item_data[] = array(
			'name'  => __( 'Date Joined', 'buddyboss' ),
			'value' => $membership->date_modified,
		);

		$data_to_export[] = array(
			'group_id'    => 'bp_groups_memberships',
			'group_label' => __( 'Group Memberships', 'buddyboss' ),
...
function bp_groups_pending_requests_personal_data_exporter( $email_address, $page ) {
	...
	foreach ( $requests as $request ) {
		...
		$data_to_export[] = array(
			...
			'group_label' => __( 'Pending Group Membership Requests', 'buddyboss' ),
...
function bp_groups_pending_sent_invitations_personal_data_exporter( $email_address, $page ) {
	...
	foreach ( $invitations as $invitation ) {
		...
		$data_to_export[] = array(
			...
			'group_label' => __( 'Pending Group Invitations (Sent)', 'buddyboss' ),
...
function bp_groups_pending_received_invitations_personal_data_exporter( $email_address, $page ) {
	...
	foreach ( $invitations as $invitation ) {
		...
		$data_to_export[] = array(
			...
			'group_label' => __( 'Pending Group Invitations (Received)', 'buddyboss' ),
...
/**
 * Migrate invitations and requests from pre-1.2.5 group_members table to invitations table.
 *
 * @since BuddyBoss 1.2.5
 */
